...
macro_rules! ffi_dispatch(
    ...
    ($handle: ident, $func: ident, $($arg: expr),*) => (
        ...
        dlib::ffi_dispatch!("dlopen", $handle, $func, $($arg),*)
    );
);
...
macro_rules! ffi_dispatch_static(
    ...
    ($handle:ident, $name: ident) => (
        dlib::ffi_dispatch_static!("dlopen", $handle, $name);
    );
);
